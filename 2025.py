...
# ==============================================================================
# CONFIGURACIÓN DE PARÁMETROS - VERSIÓN 2025
# ==============================================================================

TITULO_APP = "LIQUIDADOR TASAS POR SERVICIOS GENERALES - EJERCICIO 2025"

FACTOR_TERRENO_HASTA_10000 = 5
FACTOR_TERRENO_MAS_10000 = 10
FACTOR_EDIFICADO = 100
VALOR_M2_PROTOTIPICO = 962.34

# Tabla 2025: (Limite Superior, Limite Inferior, Cuota Fija, Alicuota Excedente)
...
# Porcentajes de aumento para las cuotas (2025)
PORCENTAJES_AUMENTO = [
    0.0,  # Cuota 1
    0.0,  # Cuota 2
    0.0,  # Cuota 3
...
with col_sub3:
  entry_val_liq2024 = st.text_input("Última liquidación 2024 ($):", "0,00")
...
with col_val2:
  var_anio = st.selectbox("Valuación año:", ["Anterior a 2023", "2024", "2025"])
...
try:
  ...
  try:
    val_liq2024_num = (
        float(entry_val_liq2024.replace(".", "").replace(",", "."))
        if entry_val_liq2024
        else 0.0
    )
  except ValueError:
    val_liq2024_num = 0.0
  ...
  for i in range(1, 13):
    ...
    if i == 1:
      if var_tope == "NO":
        ...
        sub_c_acumulado = round(
            val_liq2024_num * (1.0 + (pct_cuota_1 / 100.0)), 2
        )
        nombre_cuota = f"CUOTA 1-2025 ({pct_cuota_1}%)"
      else:
        sub_c_acumulado = tasa_mensual
        nombre_cuota = "CUOTA 1-2025 (0%)"
    else:
      nombre_cuota = f"CUOTA {i}-2025 ({pct}%)"
      if pct > 0:
    ...
    # Regla de Edenor 2025: Sin descuento en cuotas 6, 8 y 9
    if i in [6, 8, 9]:
      m_edenor_c = 0.0
    else:
